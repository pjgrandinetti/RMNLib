...
//bool DatasetImportJCAMPIsValidURL(CFURLRef url)
//{
//    bool result = false;
//    OCStringRef extension = CFURLCopyPathExtension(url);
//    if(extension) {
//        if(OCStringCompare(extension, STR("jdx"), 0) == kOCCompareEqualTo) result = true;
//        OCRelease(extension);
//    }
//    return result;
//}
//
//OCIndex DatasetImportJCAMPNumberOfDimensionsForURL(CFURLRef url)
//{
//    return 0;
//}
...
OCDictionaryRef  DatasetImportJCAMPCreateDictionaryWithLines(OCArrayRef lines, OCIndex *index)
{
    ...
    if(OCStringCompare(key0, STR("TITLE"), 0)!=kOCCompareEqualTo) return NULL;
    ...
    for(*index = start; *index<OCArrayGetCount(lines); (*index)++) {
        ...
        if(OCArrayGetCount(array) == 2) {
            ...
            if(OCStringGetLength(key)) {
                ...
                else if(OCStringCompare(key, STR("END"), kOCCompareCaseInsensitive)==kOCCompareEqualTo) return dictionary;
                else  {
...
DatasetRef DatasetImportJCAMPCreateSignalWithData(OCDataRef contents, OCStringRef *error)
{
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        if(error) {
            OCStringRef desc = STR("JCAMP Peak Table file is unsupported.");
        }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        OCMutableStringRef string = OCStringCreateMutableCopy(0, (OCStringRef) OCDictionaryGetValue(dictionary, key));
        if(string) {
            OCStringTrimWhitespace(string);
            OCDictionaryAddValue(jcampDatasetMetaData, key, string);
            if(OCStringCompare(string, STR("1/CM"), 0)== kOCCompareEqualTo) {
                double unit_multiplier = 1;
                xUnits = SIUnitForParsedSymbol(STR("1/cm"), &unit_multiplier, error);
                inverseXUnits = SIUnitForParsedSymbol(STR("cm"), &unit_multiplier, error);
            ...
            if(OCStringCompare(string, STR("VOLUME"), 0)== kOCCompareEqualTo) {
                double unit_multiplier = 1;
                xUnits = SIUnitForParsedSymbol(STR("mL"), &unit_multiplier, error);
                inverseXUnits = SIUnitForParsedSymbol(STR("1/mL"), &unit_multiplier, error);
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("Th"), &unit_multiplier, error);
                inverseXUnits = SIUnitByParsingSymbol(STR("(1/Th)"), &unit_multiplier, error);
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("nm"), &unit_multiplier, error);
                inverseXUnits = SIUnitForParsedSymbol(STR("1/nm"), &unit_multiplier, error);
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("G"), &unit_multiplier, error);
                inverseXUnits = SIUnitForParsedSymbol(STR("1/G"), &unit_multiplier, error);
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("Hz"), &unit_multiplier, error);
                inverseXUnits = SIUnitForSymbol(STR("s"));
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("min"), &unit_multiplier, error);
                inverseXUnits = SIUnitForSymbol(STR("Hz"));
                ...
                double unit_multiplier = 1;
                xUnits = SIUnitByParsingSymbol(STR("s"), &unit_multiplier, error);
                inverseXUnits = SIUnitForSymbol(STR("Hz"));
                quantityName = kSIQuantityTime;
                inverseQuantityName = kSIQuantityFrequency;
            }
...
//    SIUnitRef yUnits = NULL;
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        resolution = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        firstX = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        lastX = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        deltaX = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        maxY = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        minY = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        xFactor = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        yFactor = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        size = OCStringGetIntValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        firstY = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) OCDictionaryAddValue(jcampDatasetMetaData, key, string);
        observeFrequency = OCStringGetDoubleValue(string);
    }
    ...
    if(OCDictionaryContainsKey(dictionary, key)) {
        ...
        OCMutableArrayRef dataLines = OCArrayCreateMutableCopy(0, (OCMutableArrayRef) [(NSString *) string componentsSeparatedByString:@"\n"]);
        
        OCIndex i=0;
        for(OCIndex index = 1; index<OCArrayGetCount(dataLines);index++) {
            OCMutableStringRef line = OCStringCreateMutableCopy(0, OCArrayGetValueAtIndex(dataLines, index));
            ...
            OCStringTrimWhitespace(line);
            OCMutableArrayRef array = OCArrayCreateMutableCopy(0, (OCMutableArrayRef) [(NSString *) line componentsSeparatedByString:@" "]);
            OCRelease(line);
            ...
            for(OCIndex jndex=0;jndex<OCArrayGetCount(array);jndex++) {
                OCMutableStringRef stringNumber = OCStringCreateMutableCopy(0, OCArrayGetValueAtIndex(array, jndex));
                int dup = 0;
                ...
                if(jndex>0) {
                    data[i] = OCStringGetDoubleValue(stringNumber);
    ...
    OCDataRef values = OCDataCreate((const UInt8 *) data, sizeof(float)*size);
    ...
    if(quantityName && OCStringCompare(quantityName,kSIQuantityFrequency,0)==kOCCompareEqualTo) {
        ...
        SIUnitRef megahertz = SIUnitForSymbol(STR("MHz"));
    ...
    if(inverseQuantityName && OCStringCompare(inverseQuantityName,kSIQuantityFrequency,0)==kOCCompareEqualTo) {
        ...
        SIUnitRef megahertz = SIUnitForUnderivedSymbol(STR("MHz"));
    ...
    SIDimensionSetOriginOffset(dim, originOffset);
    SIDimensionSetInverseOriginOffset(dim, inverseOriginOffset);
    SIDimensionSetReferenceOffset(dim, referenceOffset);
    ...
    DependentVariableRef theDependentVariable = DatasetAddDefaultDependentVariable(theDataset, STR("scalar"), kSINumberFloat32Type, kDatasetSizeFromDimensions);
    ...
    string = NULL;
    key = STR("TEMPERATURE");
    SIUnitRef unit = SIUnitByParsingSymbol(STR("°C"), NULL, error);
    quantityName = kSIQuantityTemperature;
    
    if(OCDictionaryContainsKey(dictionary, key)) {
        string = (OCStringRef) OCDictionaryGetValue(dictionary, key);
        if(string) {
            double value = OCStringGetDoubleValue(string);
    ...
    string = NULL;
    key = STR("PRESSURE");
    unit = SIUnitByParsingSymbol(STR("atm"), NULL, error);
    ...
    unit = SIUnitByParsingSymbol(STR("m•s/(m•s)"), NULL, error);
    ...
    string = NULL;
    key = STR("DENSITY");
    unit = SIUnitByParsingSymbol(STR("g/cm^3"), NULL, error);
    ...
    string = NULL;
    key = STR("MW");
    unit = SIUnitByParsingSymbol(STR("g/mol"), NULL, error);
